...
//! guards, and the `rtp` / `mux` echo servers) so each focused test target
//! stays small and readable.
//!
//! The generic helpers (payload, presets, stats reporting, prng, task_scope,
//! and the module core) now live in the harness kit behind the `test-kit`
//! feature; this module is a thin re-export view of that single authority.
//! The scenario targets that stay here are the rtp-owned suites (their move
//! into `rtp` is the relocation's step 5); the mux and rtp_mux suites
//! already relocated into their owning crates with the layer kits.
