//! Loopback perf-ceiling probes for the harness lanes: raw `rtp` 4 MiB echoes,
//! the time-boxed `mux`-over-`rtp` hostile-link goodput / sparse-message
//! latency probes that feed the perf-loop battery, plus the two seeding tests
//! that pin the controller/deterministic-iid-loss link presets.
//!
//! The loopback `mux`-over-`rtp` ceiling probes (and the transient-connect /
//! floor machinery they share) moved to the owning crate with the mux
//! scenarios: `mux/tests/perf_probe.rs`, sharing the single kit authority
//! (`mux::testkit::mux`) consumed here through the `support/mux.rs` shim.
//!
//! The tests are `#[ignore]` by default so they compile without running;
//! execute them with:
//!
//! ```sh
//! cargo test --release -p tests --test perf_probe -- --ignored --nocapture --test-threads=1
//! ```
