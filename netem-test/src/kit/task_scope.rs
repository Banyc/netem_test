...
pub type TestTask = std::pin::Pin<Box<dyn std::future::Future<Output = ()> + Send + 'static>>;
...
impl TestScope {
    /// Create an empty scope. An explicit constructor is preferred over a
    /// fabricated `Default` (the house rule for domain/validated types), so
    /// the lint asking for a `Default` impl is deliberately allowed.
    ...
    pub fn submitter(&mut self, bound: usize) -> TestTaskSubmitter {
    ...
    pub async fn run<F: Future>(mut self, body: F) -> F::Output {
        tokio::pin!(body);
        let value = loop {
...
pub fn spawn_test_task_reaper_with_shutdown(
...
pub async fn abort_and_reap_test_tasks(tasks: &mut tokio::task::JoinSet<()>) {
    tasks.abort_all();
...
pub fn submit_test_task(tx: &TestTaskSubmitter, fut: TestTask) {
...
pub fn submit_test_task_required(
